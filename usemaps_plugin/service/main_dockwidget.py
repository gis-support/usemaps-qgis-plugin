...
class MainDockWidget(QtWidgets.QDockWidget, FORM_CLASS, Logger):
    ...
    def offers_projects_populate_table(self, response: dict) -> None:
        ...
        self.offers_projects_source_model.setHorizontalHeaderLabels(["ID", "Nazwa", "Status", "Kierownik"])
        header = self.tableProjects.horizontalHeader()
    ...
    def offers_projects_apply_layers(self, response: dict, project_name: str) -> None:
        ...
        if not assigned_sources:
            self.message(self.tr(f"Projekt '{project_name}' nie posiada powiązanych źródeł danych"), level=1, duration=3)
            return

        if not candidate_layers:
            self.message(self.tr(f"Projekt '{project_name}' nie posiada warstw dostępnych dla Ciebie"), level=1, duration=3)
            return
        ...
        if loaded == 0:
            ...
            self.message(self.tr(f"Wczytano warstwy projektu: {project_name}"), duration=3)
